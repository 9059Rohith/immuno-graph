...
const credentialsSchema = z
  .object({ email: z.string().email().max(254), password: z.string().min(10).max(200) })
  .strict();
...
export function registerAuthRoutes(
  ...
  app.post('/api/v1/auth/signup', async (request, reply) => {
    ...
    if (await database.user.findUnique({ where: { email } }))
      throw new ApiError('EMAIL_IN_USE', 409, 'An account already exists for this email.');
  ...
  app.post('/api/v1/auth/login', async (request, reply) => {
    ...
    if (user === null || !(await passwordMatches(parsed.data.password, user.passwordHash)))
      throw new ApiError('INVALID_CREDENTIALS', 401, 'Email or password is incorrect.');
    reply.header('Set-Cookie', cookie(await createSession(user.id)));
    return { requestId: String(request.id), data: { user: publicUser(user) } };
  });
